...
<div id="div1">
    ...
    <div class="jems">
      <h3>Elementary School Department</h3>
    </div>
...
<br>
    ...
    <?php $itemScore = array();?>
    <?php foreach($teachers as $indexTeach => $teacher):?>
    <?php  $evaluation = Evaluation::find()->where(['eval_for' => $teacher->id])->one(); ?>
    <?php Teacher::find()->where(['user_id' => $teacher->id])->one() ?>
        ...
        <?php $sect = Section::find()->where(['instrument_id' => $inst->id]); ?>
                        <?php foreach($sect->all() as $se => $s):?>
                            ...
                            $me +=$sectItems->count()?>
                        <?php endforeach; ?>
                <?php " AVERAGE SCORE: ". $teacher->teacher->fullname."-".$averageScore = $mee/$me; ?>
                <?php $name = $teacher->teacher->fullname ?>
                <?php $data[$indexTeach]['name'] = $name?>
                <?php $data[$indexTeach]['score'] = $averageScore?>
                <?php 
                $averageScore = 0;
                $mee = 0;?>
                <?php $me = 0;?>
