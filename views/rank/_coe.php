...
<?php 
    ...
    <?php $itemScore = array();?>
    <?php foreach($coeTeachers as $indexCoeTeac => $coeTeacher):?>
    <?php  $evaluation = Evaluation::find()->where(['eval_for' => $coeTeacher->id])->one(); ?>
    <?php Teacher::find()->where(['user_id' => $coeTeacher->id])->one() ?>
                        ...
                        <?php foreach($sect->all() as $se => $s):?>
                            ...
                            <?php $sectScroore = 0; ?>
                                            <?php foreach($sectItems->all() as $secI => $sI):?> 
                                                    ...
                                                    ON evaluation.eval_for = '. $coeTeacher->id .' 
                            ...
                            $me +=$sectItems->count()?>
                        <?php endforeach; ?>
                <?php " AVERAGE SCORE: ". $coeTeacher->teacher->fullname."-".$averageScore = $mee/$me; ?>
                <?php $name = $coeTeacher->teacher->fullname ?>
                <?php $data[$indexCoeTeac]['name'] = $name?>
                <?php $data[$indexCoeTeac]['score'] = $averageScore?>
                <?php 
                $averageScore = 0;
                $mee = 0;?>
                <?php $me = 0;?>
